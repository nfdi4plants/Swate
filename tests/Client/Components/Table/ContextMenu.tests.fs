...
type TestCases =
    ...
    static member NotAddColumns() =
        let pasteData = Fixture.Body_Component_InstrumentModel_Pseudo_SingleRow_String
    ...
    static member AddSingleCell() =
        let pasteData = Fixture.Body_Component_InstrumentModel_SingleRow_String
...
let Main =
    ...
    testList "Context Menu" [
        testList "Prediction" [
            //testCase "Add term column" <| fun _ -> TestCases.AddColumns(Fixture.Component_InstrumentModel_String, Fixture.mkSelectHandle (1, 1, 3, 3), [| Fixture.Component_InstrumentModel_Term_Column |])
            ...
            <| fun _ -> TestCases.PasteMultipleCells(2, 1, Fixture.Body_Component_InstrumentModel_SingleRow_String)
            testCase $"Paste {2} Cell(s) in the same row. Paste {1} Cell(s) in the same column"
            <| fun _ -> TestCases.PasteMultipleCells(3, 1, Fixture.Body_Component_InstrumentModel_SingleRow_String)
            testCase $"Paste {3} Cell(s) in the same row. Paste {1} Cell(s) in the same column"
            <| fun _ -> TestCases.PasteMultipleCells(1, 2, Fixture.Body_Component_InstrumentModel_TwoRows_String)
            testCase $"Paste {2} Cell(s) in the same row. Paste {2} Cell(s) in the same column"
            <| fun _ -> TestCases.PasteMultipleCells(2, 2, Fixture.Body_Component_InstrumentModel_TwoRows_String)
            testCase $"Paste {3} Cell(s) in the same row. Paste {2} Cell(s) in the same column"
            <| fun _ -> TestCases.PasteMultipleCells(3, 2, Fixture.Body_Component_InstrumentModel_TwoRows_String)
            testCase $"Add fitting Term"
            <| fun _ -> TestCases.AddFittingTerm(2, 0, Fixture.Body_Component_InstrumentModel_SingleRow_Term_String)
            testCase $"Add fitting Term and 1 freetext"
            <| fun _ -> TestCases.AddFittingTerm(1, 0, Fixture.Body_Component_InstrumentModel_SingleRow_Term_String)
            testCase $"Add fitting Term and 2 freetexts"
            <| fun _ -> TestCases.AddFittingTerm(0, 0, Fixture.Body_Component_InstrumentModel_SingleRow_Term_String)
