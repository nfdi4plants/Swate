...
Vitest.describe (
    ...
    fun () ->
        Vitest.test (
            "subscribes before loading and does not overwrite a live update with a stale snapshot",
            fun () -> promise {
                ...
                let loadSnapshot () = promise {
                    ...
                    return! Promise.create (fun resolve _reject -> resolveLoad <- Some resolve)
                }
                ...
                try
                    ...
                    do!
                        waitForEffect (fun () ->
                            observed
                            |> Seq.exists (fun (state, isLoading) -> state = "live-update" && not isLoading)
                        )
