...
Vitest.describe("NoteConversion", fun () ->
    Vitest.test("formatMarkdown writes YAML frontmatter and preserves body text", fun () ->
        let tag =
    ...
    Vitest.test("frontmatter decoder returns None when tags field is absent", fun () ->
        let markdown =
            """---
title: Untagged note
...
"""
    ...
    Vitest.test("formatMarkdown omits tags for drafts without tags", fun () ->
        let draft = {
            NotesDraft.init with
                Title = "Untagged note"
                DateCreated = Some(DateTime(2026, 4, 27))
