...
let private withTempArc = TestHelpers.withTempArcWith "swate-delete-arc-file-" "DeleteArcFileArc"
...
let private assertDeletedOnDisk arcPath relativeParts containsEntity =
    promise {
...
Vitest.describe("ARC delete helper", fun () ->
    Vitest.test("deletes an assay and removes its folder", fun () ->
    ...
    Vitest.test("deletes an entity when given the canonical workbook path", fun () ->
        withTempArc (fun arc -> arc.AddAssay(ArcAssay("DeleteAssay"))) (fun arcPath -> promise {
            let! arc = loadArcAsync arcPath
    ...
    Vitest.test("deletes a study and removes its folder", fun () ->
        withTempArc (fun arc -> arc.AddStudy(ArcStudy("DeleteStudy"))) (fun arcPath -> promise {
            let! arc = loadArcAsync arcPath
            ...
            do! assertDeletedOnDisk arcPath [| "studies"; "DeleteStudy" |] (fun arc ->
                arc.ContainsStudy("DeleteStudy"))
        }))

    Vitest.test("deletes a run and removes its folder", fun () ->
        withTempArc (fun arc -> arc.AddRun(ArcRun("DeleteRun"))) (fun arcPath -> promise {
            let! arc = loadArcAsync arcPath
            let! deletedArc = deleteEntityOrFail arcPath "runs/DeleteRun" arc

            Vitest.expect(deletedArc.ContainsRun("DeleteRun")).toBe(false)
            do! assertDeletedOnDisk arcPath [| "runs"; "DeleteRun" |] (fun arc ->
                arc.ContainsRun("DeleteRun"))
        }))

    Vitest.test("deletes a workflow and removes its folder", fun () ->
        withTempArc (fun arc -> arc.AddWorkflow(ArcWorkflow("DeleteWorkflow"))) (fun arcPath -> promise {
            let! arc = loadArcAsync arcPath
            ...
            do! assertDeletedOnDisk arcPath [| "workflows"; "DeleteWorkflow" |] (fun arc ->
                arc.ContainsWorkflow("DeleteWorkflow"))
        }))

    Vitest.test("rejects missing entity identifiers without mutating the in-memory ARC", fun () ->
        withTempArc (fun arc -> arc.AddAssay(ArcAssay("KeepAssay"))) (fun arcPath -> promise {
            let! arc = loadArcAsync arcPath
    ...
    Vitest.test("deletes through the scoped ARC path without persisting unrelated dirty in-memory edits", fun () ->
        withTempArc
            (fun arc ->
