...
Vitest.describe("study events", fun () ->
    Vitest.test("add event: new disc study appears in merged result", fun () ->
        let arcLocal, arcRemote = MockData.createTwoCleanCopies ()
    ...
    Vitest.test("change event: disc study replaces local study", fun () ->
    ...
    Vitest.test("change event: in-memory DataMap is forwarded onto disc study when no datamap event exists", fun () ->
        let arcLocal, arcRemote = withStudy ()
    ...
    Vitest.test("study not targeted by event keeps in-memory state", fun () ->
        let arcLocal, arcRemote = withStudy ()
    ...
    Vitest.test("unlink event: study is removed from local", fun () ->
        let arcLocal, arcRemote = withStudy ()
        arcLocal.Title <- Some "User Title"
