...
Vitest.describe("run events", fun () ->
    Vitest.test("add event: new disc run appears in merged result", fun () ->
        let arcLocal, arcRemote = MockData.createTwoCleanCopies ()
    ...
    Vitest.test("change event: disc run replaces local run", fun () ->
    ...
    Vitest.test("change event: in-memory DataMap is forwarded onto disc run when no datamap event exists", fun () ->
        let arcLocal, arcRemote = withRun ()
    ...
    Vitest.test("run not targeted by event keeps in-memory state", fun () ->
        let arcLocal, arcRemote = withRun ()
    ...
    Vitest.test("unlink event: run is removed from local", fun () ->
        let arcLocal, arcRemote = withRun ()
        arcLocal.Title <- Some "User Title"
