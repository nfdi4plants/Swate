...
Vitest.describe (
    ...
    fun () ->
        Vitest.test (
            "clean local: returns copy of remote",
            fun () ->
        ...
        Vitest.test (
            "dirty local with empty event list: returns copy of local",
            fun () ->
                let arcLocal, arcRemote = MockData.createTwoCleanCopies ()
        ...
        Vitest.test (
            "clean local with events: returns copy of remote, events are ignored",
            fun () ->
                let arcLocal, arcRemote = MockData.createTwoCleanCopies ()
                arcRemote.Title <- Some "Disc Title"
        ...
        Vitest.test (
            "dirty local with events: processes events without mutating inputs",
            fun () ->
                let arcLocal, arcRemote = MockData.createTwoCleanCopies ()
                arcLocal.Title <- Some "User Title"
                arcRemote.Title <- Some "Disc Title"
