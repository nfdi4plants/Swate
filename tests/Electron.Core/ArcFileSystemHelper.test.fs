...
let private acceptImportedFiles () = promise { return Ok() }
...
Vitest.describe (
    ...
    fun () ->
        ...
        Vitest.test (
            "rolls back imported files when post-import validation fails",
            fun () ->
                withAssayArc (fun arcPath -> promise {
                    let sourceDirectory = dirname arcPath
