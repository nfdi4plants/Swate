...
let rec private flattenNodes (nodes: ArcExplorerNode list) = seq {
    for node in nodes do
        yield node
        yield! flattenNodes node.children
}

let rec private flattenFileItems (items: FileItem list) = seq {
...
let private isLeafRehomeCandidateAtSiblingLevel (item: FileItem) =
    let hasMarker (marker: string) = item.Id.Contains marker
...
let private isWhitelistedGraphKind (metaById: Map<string, GraphNodeMeta>) (node: ArcExplorerNode) =
    graphKindForNode metaById node |> whitelistedGraphKinds.Contains
...
let private graphOptionIndexByLabel (label: string) =
    ...
    |> expectSome
    <| $"Missing graph kind option '{label}'."

let private graphOptionIndicesByLabels (labels: string list) =
    labels |> List.map graphOptionIndexByLabel |> Set.ofList

let private sortedIndices (indices: Set<int>) = indices |> Seq.sort |> Seq.toList

let private extractEntryCount (metaById: Map<string, GraphNodeMeta>) (layerId: string) =
    metaById |> Map.tryFind layerId |> expectSome
    <| $"Missing metadata for {layerId}."
    |> fun layerMeta ->
        layerMeta.Rows
        ...
        |> expectSome
        <| $"Missing numeric Entries row in metadata for {layerId}."
...
let private materialEndpointsFromMaterials (materials: Material array) =
    materials |> Array.toList |> List.map MaterialValue

let private dataEndpointsFromData (dataValues: Data array) =
    dataValues |> Array.toList |> List.map DataValue
...
let private endpointIdentity =
    function
    | MaterialValue material -> String.Join("|", [ "material"; material.id; material.name; material.type' ])
    | DataValue data ->
        String.Join(
            "|",
            [
...
let private collectSourceStats (graphObjects: ARCObjects list) =
    ...
    let rec addProtocol (protocol: LabProtocol) =
        ...
        protocol.processes |> Array.iter addProcess
    ...
    and addProcess (processValue: LabProcess) =
        ...
        let unassociatedEndpoints = unassociatedProcessEndpoints processValue
    ...
    |> List.iter (
        ...
        | ARCObjects.Arc arcs ->
            ...
            |> Array.iter (fun arcGraph ->
                ...
                arcGraph.Datasets |> flattenDatasetKinds |> List.iter addDataset
            )
        | _ -> ()
    )
...
let private createPropertyValue
    (id: string, name: string, value: string option, additionalType: string option)
    : PropertyValue =
    {
        id = id
        type' = "PropertyValue"
...
let private graphObjectsWithPropertyValues () : ARCObjects list =
    let datasetProperty =
        createPropertyValue ("prop:dataset-region", "Dataset Region", Some "Field-01", Some "Geospatial")

    let protocolProperty =
        createPropertyValue ("prop:protocol-revision", "Protocol Revision", Some "2026-04", Some "RevisionTag")

    let processParameterValue =
        createPropertyValue ("prop:process-batch", "Batch", Some "B-42", Some "BatchNumber")

    let materialProperty =
        createPropertyValue ("prop:material-origin", "Material Origin", Some "Greenhouse", Some "OriginTag")

    let dataProperty =
        createPropertyValue ("prop:data-format", "Data Format", Some "tsv", Some "MimeHint")
...
let private graphObjectsWithPropertyRowParity () : ARCObjects list =
    let sharedProperty =
        createPropertyValue ("prop:shared", "   ", Some "Shared Value", Some "SharedType")
...
Vitest.describe (
    "ToArcExplorerNodes graph conversion",
    fun () ->
        Vitest.test (
            "uses semantic graph filter options in storybook",
            fun () ->
                let labels =
                    KindFilter.graphObjectExplorerOptions |> Array.map _.item |> Array.toList
        ...
        Vitest.test (
            "syncs dataset child kinds when Datasets is toggled on",
            fun () ->
                ...
                let datasetChildIndices =
                    graphOptionIndicesByLabels [ "Study"; "Assay"; "Workflow"; "Run" ]
                ...
                let expectedSelection = Set.union nextSelection datasetChildIndices

                Vitest.expect(sortedIndices syncedSelection).toEqual (sortedIndices expectedSelection)
        )

        Vitest.test (
            "syncs dataset child kinds when Datasets is toggled off",
            fun () ->
                let datasetIndex = graphOptionIndexByLabel "Datasets"
                let protocolsIndex = graphOptionIndexByLabel "Protocols"
                ...
                let expectedSelection = Set.difference nextSelection datasetChildIndices

                Vitest.expect(sortedIndices syncedSelection).toEqual (sortedIndices expectedSelection)
        )

        Vitest.test (
            "keeps Datasets unchanged when only child kinds are toggled",
            fun () ->
                let datasetIndex = graphOptionIndexByLabel "Datasets"
                ...
                let previousSelection = Set.union (Set.singleton datasetIndex) datasetChildIndices

                let nextSelection = previousSelection |> Set.remove studyIndex
        ...
        Vitest.test (
            "keeps select-all and clear-all idempotent with dataset sync",
            fun () ->
                let allIndices =
                    KindFilter.defaultSelectedIndices KindFilter.graphObjectExplorerOptions
        ...
        Vitest.test (
            "ignores unknown semantic labels while filtering",
            fun () ->
        ...
        Vitest.test (
            "builds expected top-level shape for graph explorer and omits standalone roots",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                ...
                let datasetsLayer =
                    nodes |> List.tryFind (fun node -> node.id = "graph:datasets") |> expectSome
                    <| "Missing datasets layer."

                Vitest.expect(datasetsLayer.children.Length > 0).toBe (true)

                let allLayer =
                    nodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing all layer."
        ...
        Vitest.test (
            "flattens assay descendants into structural folders while keeping folder descendants expandable",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                ...
                let assayNode =
                    ...
                    |> expectSome
                    <| "Expected Metabolomics assay node in flattened tree."

                let assayChildren = assayNode.Children |> Option.defaultValue []

                let protocolNodesAtAssayLevel =
                    assayChildren |> List.filter (fun item -> item.Name = "LC-MS Measurement")

                let processNodesAtAssayLevel =
                    assayChildren |> List.filter (fun item -> item.Name = "Quantify features")

                let protocolsGroup =
                    assayChildren
                    |> List.tryFind (fun item -> item.Name = "Protocols")
                    |> expectSome
                    <| "Expected structural Protocols group at assay parent level."

                let processesGroup =
                    assayChildren
                    |> List.tryFind (fun item -> item.Name = "Processes")
                    |> expectSome
                    <| "Expected structural Processes group at assay parent level."

                let inputsGroup =
                    assayChildren |> List.tryFind (fun item -> item.Name = "Inputs") |> expectSome
                    <| "Expected structural Inputs group at assay parent level."

                let outputsGroup =
                    assayChildren |> List.tryFind (fun item -> item.Name = "Outputs") |> expectSome
                    <| "Expected structural Outputs group at assay parent level."

                let inputsAtAssayLevel =
                    assayChildren |> List.filter (fun item -> item.Name = "Inputs")

                let outputsAtAssayLevel =
                    assayChildren |> List.filter (fun item -> item.Name = "Outputs")
                ...
                let protocolNodeInProtocolsGroup =
                    ...
                    |> expectSome
                    <| "Expected protocol node inside Protocols group."
                ...
                let processNodeInProcessesGroup =
                    ...
                    |> expectSome
                    <| "Expected process node inside Processes group."
        ...
        Vitest.test (
            "merges repeated Inputs and Outputs folders on flattened ARC root level",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, nodeMetaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                let explorerItems = toGraphFileItems nodeMetaById nodes
                ...
                let arcsRoot =
                    treePaneItems |> List.tryFind (fun item -> item.Id = "graph:all") |> expectSome
                    <| "Expected ARCs root node in flattened tree."

                let arcsChildren = arcsRoot.Children |> Option.defaultValue []

                let inputFolders = arcsChildren |> List.filter (fun item -> item.Name = "Inputs")

                let outputFolders = arcsChildren |> List.filter (fun item -> item.Name = "Outputs")
                ...
                let mergedInputChildren = mergedInputs.Children |> Option.defaultValue []

                let mergedOutputChildren = mergedOutputs.Children |> Option.defaultValue []
                ...
                let mergedInputMaterialFolders =
                    mergedInputChildren
                    |> List.filter (fun item -> item.ItemType = groupItemType && item.Name = "Material")

                let mergedInputDataFolders =
                    mergedInputChildren
                    |> List.filter (fun item -> item.ItemType = groupItemType && item.Name = "Data")
                ...
                let inputSiblingLeafCandidates =
                    mergedInputChildren |> List.filter isLeafRehomeCandidateAtSiblingLevel

                let outputSiblingLeafCandidates =
                    mergedOutputChildren |> List.filter isLeafRehomeCandidateAtSiblingLevel
                ...
                Vitest
                    .expect(
                        mergedInputMaterialChildren
                        |> List.exists (fun item -> item.ItemType = materialItemType)
                    )
                    .toBe (true)

                Vitest
                    .expect(
                        mergedInputDataChildren
                        |> List.exists (fun item -> item.ItemType = dataItemType)
                    )
                    .toBe (true)

                Vitest
                    .expect(
                        mergedOutputMaterialChildren
                        |> List.exists (fun item -> item.ItemType = materialItemType)
                    )
                    .toBe (true)
        ...
        Vitest.test (
            "keeps direct ARC children first on flattened ARCs root before nested folders",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, nodeMetaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                let explorerItems = toGraphFileItems nodeMetaById nodes
                ...
                let arcChildren = arcsChildren |> List.filter isDirectArcChild

                let firstNestedChildIndex =
                    arcsChildren |> List.tryFindIndex (isDirectArcChild >> not)

                Vitest.expect(arcChildren.Length > 0).toBe (true)

                match firstNestedChildIndex with
                | Some index -> Vitest.expect(index).toBe (arcChildren.Length)
                | None -> Vitest.expect(arcsChildren.Length).toBe (arcChildren.Length)
        )

        Vitest.test (
            "fuses repeated non-directional group folders on flattened ARC root level",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, nodeMetaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                let explorerItems = toGraphFileItems nodeMetaById nodes
                ...
                let protocolGroupsAtRoot =
                    arcsChildren
                    |> List.filter (fun item -> item.ItemType = groupItemType && item.Name = "Protocols")

                let processGroupsAtRoot =
                    arcsChildren
                    |> List.filter (fun item -> item.ItemType = groupItemType && item.Name = "Processes")

                let additionalPropertiesGroupsAtRoot =
                    arcsChildren
                    |> List.filter (fun item -> item.ItemType = groupItemType && item.Name = "Additional Properties")
        ...
        Vitest.test (
            "summarizes nested additional properties into one parent-level folder in flattened tree view",
            fun () ->
                ...
                let studyNode =
                    ...
                    |> expectSome
                    <| "Expected Property Values Study node in flattened tree."
                ...
                let summaryFolder = summaryFolders |> List.head

                let summarizedAdditionalProperties =
                    summaryFolder.Children |> Option.defaultValue []

                let summarizedAdditionalPropertyNames =
                    summarizedAdditionalProperties |> List.map _.Name
                ...
                Vitest
                    .expect(
                        summarizedAdditionalProperties
                        |> List.forall (fun item -> item.Id.Contains(":additional-property:"))
                    )
                    .toBe (true)
        ...
        Vitest.test (
            "summarizes nested parameter values into one parent-level folder in flattened tree view",
            fun () ->
                let graphObjects = graphObjectsWithPropertyValues ()
                let nodes, nodeMetaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                let explorerItems = toGraphFileItems nodeMetaById nodes
                ...
                Vitest
                    .expect(
                        summarizedParameterValues
                        |> List.forall (fun item -> item.Id.Contains(":parameter-value:"))
                    )
                    .toBe (true)
        ...
        Vitest.test (
            "summarizes nested formal parameters into one parent-level folder in flattened tree view",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, nodeMetaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                let explorerItems = toGraphFileItems nodeMetaById nodes
                ...
                let parentWithFormalSummary =
                    ...
                    |> Seq.tryFind (fun item ->
                        ...
                        |> List.exists (fun child ->
                            child.Id.EndsWith(":group:flattened-formal-parameters", StringComparison.Ordinal)
                        )
                    )
                    |> expectSome
                    <| "Expected at least one flattened parent with a Formal Parameters summary folder."
                ...
                Vitest
                    .expect(
                        summarizedFormalParameters
                        |> List.forall (fun item -> item.Id.Contains(":formal-parameter:"))
                    )
                    .toBe (true)
        )

        Vitest.test (
            "shows only selected semantic top-level layer alongside ARCs root",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects

                let filteredNodes =
                    GraphObjectExplorerFilter.filterNodesBySemanticKinds (Set.ofList [ "Datasets" ]) nodes metaById None

                let topLevelIds = filteredNodes |> List.map _.id

                Vitest.expect(topLevelIds).toEqual ([ "graph:all"; "graph:datasets" ])
        ...
        Vitest.test (
            "keeps only whitelisted graph kinds in filtered graph:all and category layers",
            fun () ->
                let graphObjects = graphObjectsWithPropertyValues ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                let filteredNodes =
                    GraphObjectExplorerFilter.filterNodesBySemanticKinds selectedSemanticKinds nodes metaById None

                let allLayer =
                    filteredNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing graph:all layer while checking whitelist."
                ...
                let allFilteredKinds = Set.union allLayerKinds categoryLayerKinds

                allFilteredKinds
                |> Set.iter (fun graphKind -> Vitest.expect(whitelistedGraphKinds.Contains graphKind).toBe (true))
        )

        Vitest.test (
            "restores structural folders including Material and Data summaries in graph:all",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                let allLayer =
                    filteredNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing graph:all layer while checking structural folders."
        ...
        Vitest.test (
            "shows study datasets without exposing datasets top-level layer",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects

                let filteredNodes =
                    GraphObjectExplorerFilter.filterNodesBySemanticKinds (Set.ofList [ "Study" ]) nodes metaById None

                let topLevelIds = filteredNodes |> List.map _.id

                Vitest.expect(topLevelIds).toEqual ([ "graph:all" ])

                Vitest.expect(topLevelIds |> List.contains "graph:datasets").toBe (false)

                let allLayer =
                    filteredNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing graph:all layer after Study semantic filtering."

                let canonicalNodes = allLayer.children |> flattenNodes |> Seq.toList

                let datasetNodes =
                    canonicalNodes |> List.filter (hasTag metaById GraphNodeTag.Dataset)

                let studyNodes =
                    datasetNodes |> List.filter (fun node -> node.kind = ArcExplorerNodeKind.Study)

                let assayNodes =
                    datasetNodes |> List.filter (fun node -> node.kind = ArcExplorerNodeKind.Assay)
                ...
                let runNodes =
                    datasetNodes |> List.filter (fun node -> node.kind = ArcExplorerNodeKind.Run)
        ...
        Vitest.test (
            "keeps only assay datasets in canonical and datasets layers for Datasets + Assay",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                let datasetNodesInLayer (layerId: string) =
                    filteredNodes |> List.tryFind (fun node -> node.id = layerId) |> expectSome
                    <| $"Missing {layerId} layer after semantic filtering."
                    |> fun layer ->
                        layer.children
                        |> flattenNodes
                        |> Seq.toList
        ...
        Vitest.test (
            "prunes hidden dataset branches from graph:all for Protocols-only filtering",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                Vitest.expect(topLevelIds).toEqual ([ "graph:all"; "graph:protocols" ])

                let allLayer =
                    filteredNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing graph:all layer after Protocols-only filtering."

                let allLayerCanonicalNodes = allLayer.children |> flattenNodes |> Seq.toList
                ...
                let protocolsLayer =
                    ...
                    |> expectSome
                    <| "Missing graph:protocols layer after Protocols-only filtering."
        ...
        Vitest.test (
            "keeps selected visible protocol and only its ancestor chain in graph:all",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                let selectedProtocol =
                    nodes
                    |> flattenNodes
                    |> Seq.tryFind (fun node -> not node.isReference && hasTag metaById GraphNodeTag.Protocol node)
                    |> expectSome
                    <| "Expected canonical protocol node for selected-path filtering."

                let _, selectedAncestors =
                    tryGetNodeLineageById selectedProtocol.id nodes |> expectSome
                    <| $"Missing lineage for selected protocol {selectedProtocol.id}."
                ...
                let allLayer =
                    filteredNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing graph:all layer after selected-path filtering."

                let allLayerNodes = allLayer.children |> flattenNodes |> Seq.toList

                let allLayerNodeIds = allLayerNodes |> List.map _.id |> Set.ofList

                selectedPathNodeIdsInAllLayer
                |> Set.iter (fun nodeId -> Vitest.expect(allLayerNodeIds.Contains nodeId).toBe (true))

                let visibleProtocolNodesInAllLayer =
                    allLayerNodes |> List.filter (hasTag metaById GraphNodeTag.Protocol)
        ...
        Vitest.test (
            "does not preserve hidden selected nodes and keeps resolved selection empty",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                let selectedStudy =
                    ...
                    |> expectSome
                    <| "Expected canonical study node for hidden-selection regression."
                ...
                let allLayer =
                    filteredNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing graph:all layer after hidden-selection filtering."

                let allLayerNodeIds = allLayer.children |> flattenNodes |> Seq.map _.id |> Set.ofSeq
        ...
        Vitest.test (
            "isolates endpoint categories without kind-label leakage",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                Vitest.expect(topLevelIds).toEqual ([ "graph:all"; "graph:materials" ])

                let allLayer =
                    filteredNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing graph:all layer after semantic filtering."

                let canonicalNodes = allLayer.children |> flattenNodes |> Seq.toList

                let materialEndpointTag =
                ...
                let dataEndpointNodeCount =
                    canonicalNodes |> List.filter (hasTag metaById dataEndpointTag) |> List.length

                let materialsLayer =
                    filteredNodes
                    |> List.tryFind (fun node -> node.id = "graph:materials")
                    |> expectSome
                    <| "Missing materials layer after semantic filtering."
        ...
        Vitest.test (
            "ignores non-Arc ARCObjects entries when creating the canonical tree",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                ...
                let baselineAllLayer =
                    baselineNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing baseline graph:all layer."

                let noisyAllLayer =
                    noisyNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                    <| "Missing noisy graph:all layer."

                Vitest.expect(noisyAllLayer.children.Length).toBe (baselineAllLayer.children.Length)

                let baselineDatasetsLayer =
                    baselineNodes
                    |> List.tryFind (fun node -> node.id = "graph:datasets")
                    |> expectSome
                    <| "Missing baseline datasets layer."

                let noisyDatasetsLayer =
                    noisyNodes
                    |> List.tryFind (fun node -> node.id = "graph:datasets")
                    |> expectSome
                    <| "Missing noisy datasets layer."

                Vitest.expect(noisyDatasetsLayer.children.Length).toBe (baselineDatasetsLayer.children.Length)
        )

        Vitest.test (
            "emits metadata labels for canonical datasets, processes, and process endpoints",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                let datasetNode =
                    nodes
                    |> flattenNodes
                    |> Seq.tryFind (fun node -> node.kind = ArcExplorerNodeKind.Study && node.isReference = false)
                    |> expectSome
                    <| "Expected canonical dataset node."

                let datasetMeta =
                    metaById |> Map.tryFind datasetNode.id |> expectSome
                ...
                let processMeta =
                    ...
                    |> expectSome
                    <| "Expected at least one process metadata entry."
                ...
                Vitest
                    .expect(
                        processMeta.Rows
                        |> List.exists (fun (label, value) -> label = "Type" && value = "LabProcess")
                    )
                    .toBe (true)
                ...
                let protocolMeta =
                    ...
                    |> expectSome
                    <| "Expected at least one protocol metadata entry."
                ...
                let formalParameterMeta =
                    ...
                    |> expectSome
                    <| "Expected at least one formal parameter metadata entry."
                ...
                let materialMeta =
                    ...
                    |> Seq.tryFind (fun meta ->
                        meta.Tag = Some(GraphNodeTag.ProcessEndpoint GraphProcessEndpointValueType.Material)
                    )
                    |> expectSome
                    <| "Expected at least one material endpoint metadata entry."
                ...
                let dataMeta =
                    ...
                    |> Map.values
                    |> Seq.tryFind (fun meta ->
                        meta.Tag = Some(GraphNodeTag.ProcessEndpoint GraphProcessEndpointValueType.Data)
                    )
                    |> expectSome
                    <| "Expected at least one data endpoint metadata entry."
                ...
                let propertyValueMeta =
                    ...
                    |> expectSome
                    <| "Expected at least one property-value metadata entry."
                ...
                let datasetsLayerMeta =
                    metaById |> Map.tryFind "graph:datasets" |> expectSome
                    <| "Expected metadata for datasets top-level layer."

                Vitest.expect(datasetsLayerMeta.Description.IsSome).toBe (true)
                Vitest.expect(datasetsLayerMeta.Rows |> List.exists (fun (label, _) -> label = "Entries")).toBe (true)
        )

        Vitest.test (
            "renders Inputs, Outputs, Unassociated, and Parameter Values endpoint groups for process nodes",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                |> List.iter (fun processNode ->
                    Vitest
                        .expect(processNode.children |> List.map _.name)
                        .toEqual ([ "Inputs"; "Outputs"; "Unassociated"; "Parameter Values" ])

                    let inputsGroup =
                        processNode.children
                        |> List.tryFind (fun child -> child.name = "Inputs")
                        |> expectSome
                        <| "Missing Inputs group on process node."

                    let outputsGroup =
                        processNode.children
                        |> List.tryFind (fun child -> child.name = "Outputs")
                        |> expectSome
                        <| "Missing Outputs group on process node."

                    Vitest.expect(inputsGroup.children |> List.map _.name).toEqual ([ "Material"; "Data" ])

                    Vitest.expect(outputsGroup.children |> List.map _.name).toEqual ([ "Material"; "Data" ])
                )
        )

        Vitest.test (
            "keeps category-layer counts aligned with aggregated source counts across ARCObjects",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                ...
                let datasetNodeCount =
                    canonicalNodes
                    |> List.filter (hasTag metaById GraphNodeTag.Dataset)
                    |> List.length

                let protocolNodeCount =
                    canonicalNodes
                    |> List.filter (hasTag metaById GraphNodeTag.Protocol)
                    |> List.length

                let formalParameterNodeCount =
                    canonicalNodes
                    |> List.filter (hasTag metaById GraphNodeTag.FormalParameter)
                    |> List.length

                let processNodeCount =
                    canonicalNodes
                    |> List.filter (hasTag metaById GraphNodeTag.Process)
                    |> List.length
                ...
                let protocolsLayer =
                    nodes |> List.tryFind (fun node -> node.id = "graph:protocols") |> expectSome
                    <| "Missing protocols layer."

                let formalParametersLayer =
                    nodes
                    |> List.tryFind (fun node -> node.id = "graph:formal-parameters")
                    |> expectSome
                    <| "Missing formal-parameters layer."

                let processesLayer =
                    nodes |> List.tryFind (fun node -> node.id = "graph:processes") |> expectSome
                    <| "Missing processes layer."

                let materialsLayer =
                    nodes |> List.tryFind (fun node -> node.id = "graph:materials") |> expectSome
                    <| "Missing materials layer."

                let dataLayer =
                    nodes |> List.tryFind (fun node -> node.id = "graph:Data") |> expectSome
                    <| "Missing data layer."
        ...
        Vitest.test (
            "covers supported dataset and process endpoint bucket labels on metadata",
            fun () ->
                let graphObjects = fakeGraphObjects ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                let datasetsLayerMeta =
                    metaById |> Map.tryFind "graph:datasets" |> expectSome
                    <| "Missing graph:datasets metadata."

                let datasetCaseLabels = datasetsLayerMeta.CaseExamples |> List.map fst |> Set.ofList
                ...
                |> List.iter (fun meta ->
                    Vitest
                        .expect(
                            ...
                            |> List.exists (fun (label, value) ->
                                label = "Material DU Cases" && value = "Sources | Samples"
                            )
                        )
                        .toBe (true)
                    ...
                    Vitest
                        .expect(
                            meta.Rows
                            |> List.exists (fun (label, value) ->
                                label = "Data DU Cases" && value = "Files | FragmentSelector"
                            )
                        )
                        .toBe (true)
                    ...
                    let valueType =
                        meta.Rows
                        |> List.tryPick (fun (label, value) -> if label = "Value Type" then Some value else None)
                        |> expectSome
                        <| "Missing Value Type row on endpoint metadata."
                    ...
                    | _ -> failwith $"Unexpected endpoint Value Type: {valueType}"
                )
        )

        Vitest.test (
            "normalizes whitespace consistently for direct and optional text fields",
            fun () ->
                ...
                let canonicalArcNode =
                    allLayer.children |> List.tryHead |> expectSome
                    <| "Expected canonical ARC node."

                let arcMeta =
                    metaById |> Map.tryFind canonicalArcNode.id |> expectSome
                    <| $"Expected metadata for ARC node '{canonicalArcNode.id}'."

                let canonicalNodes = allLayer.children |> flattenNodes |> Seq.toList

                let findCanonicalNodeByTag (tag: GraphNodeTag) (errorMessage: string) =
                    canonicalNodes
                    |> List.tryFind (fun node -> not node.isReference && hasTag metaById tag node)
                    |> expectSome
                    <| errorMessage

                let findMetaForNode (node: ArcExplorerNode) (errorMessage: string) =
                    metaById |> Map.tryFind node.id |> expectSome <| errorMessage
                ...
                |> List.iter (fun label ->
                    Vitest
                        .expect(protocolMeta.Rows |> List.exists (fun (rowLabel, _) -> rowLabel = label))
                        .toBe (false)
                )
                ...
                Vitest
                    .expect(
                        protocolMeta.Rows
                        |> List.exists (fun (label, value) -> label = "Intended Use" && value = "term:intended-use")
                    )
                    .toBe (true)
                ...
                Vitest
                    .expect(processMeta.Rows |> List.exists (fun (label, _) -> label = "Additional Type"))
                    .toBe (false)
        )

        Vitest.test (
            "formats property rows equally for array and option property sources",
            fun () ->
                ...
                let findRowValue (rowLabel: string) (rows: (string * string) list) =
                    rows
                    |> List.tryPick (fun (label, value) -> if label = rowLabel then Some value else None)

                let datasetMeta =
                    findCanonicalMetaByTag GraphNodeTag.Dataset "Expected metadata for canonical dataset node."

                let protocolMeta =
                    findCanonicalMetaByTag GraphNodeTag.Protocol "Expected metadata for canonical protocol node."

                let processMeta =
                    findCanonicalMetaByTag GraphNodeTag.Process "Expected metadata for canonical process node."

                let datasetAdditionalProperty =
                    datasetMeta.Rows |> findRowValue "Additional Property" |> expectSome
                    <| "Expected dataset Additional Property row."

                let protocolAdditionalProperty =
                    protocolMeta.Rows |> findRowValue "Additional Property" |> expectSome
                    <| "Expected protocol Additional Property row."

                let processParameterValue =
                    processMeta.Rows |> findRowValue "Parameter Value" |> expectSome
                    <| "Expected process Parameter Value row."
        ...
        Vitest.test (
            "adds grouped PropertyValue nodes with deterministic ids for all supported owners",
            fun () ->
                let graphObjects = graphObjectsWithPropertyValues ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                let expectPropertyGroup
                    ...
                    let groupNode =
                        ...
                        |> expectSome
                        <| $"Missing '{groupLabel}' child group on '{ownerNode.id}'."
                    ...
                    let valueNode =
                        groupNode.children |> List.tryHead |> expectSome
                        <| $"Missing PropertyValue node in group '{groupNode.id}'."

                    Vitest.expect(valueNode.id).toBe ($"{ownerNode.id}:{fieldKey}:0")
                    Vitest.expect(valueNode.isSelectable).toBe (true)

                    let valueMeta =
                        metaById |> Map.tryFind valueNode.id |> expectSome
                        <| $"Missing metadata for PropertyValue node '{valueNode.id}'."
                    ...
                    Vitest
                        .expect(
                            valueMeta.Rows
                            |> List.exists (fun (label, value) -> label = "Owner Kind" && value = expectedOwnerKind)
                        )
                        .toBe (true)
                    ...
                    Vitest
                        .expect(
                            valueMeta.Rows
                            |> List.exists (fun (label, value) -> label = "Source Field" && value = expectedSourceField)
                        )
                        .toBe (true)
        ...
        Vitest.test (
            "hides PropertyValue nodes from semantic filter output due to whitelist",
            fun () ->
                let graphObjects = graphObjectsWithPropertyValues ()
                let nodes, metaById = toArcExplorerNodesWithMetaFromArcObjects graphObjects
                ...
                let propertyValueCountForSemanticKind (semanticKind: string) =
                    ...
                    let allLayer =
                        filteredNodes |> List.tryFind (fun node -> node.id = "graph:all") |> expectSome
                        <| $"Missing graph:all layer for semantic kind '{semanticKind}'."
