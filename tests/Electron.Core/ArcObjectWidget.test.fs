...
Vitest.describe("ARCObjectWidgetHelper.GetExplorerItems", fun () ->
    Vitest.test("returns selectable descendants recursively and skips structural groups", fun () ->
        ...
        Vitest.expect(result.ContextItems |> List.filter (fun item -> item.IsCurrent) |> List.map (fun item -> item.Item.Id)).toEqual([ "study" ])
        Vitest.expect(result.Sections |> List.map (fun section -> section.Label)).toEqual([ "Table / Note"; "Sample" ])
        Vitest.expect(result.Sections.[0].Items |> List.map (fun item -> item.Item.Id)).toEqual([ "table"; "note" ])
        ...
        Vitest.expect(result.Sections |> List.collect (fun section -> section.Items) |> List.exists (fun entry -> entry.Item.Id = "tables-group")).toBe(false)
