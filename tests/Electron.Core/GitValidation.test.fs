...
Vitest.describe("GitService.ensureValidBranchLikeName", fun () ->
    ...
    for testName, value in validCases do
        Vitest.test(testName, fun () ->
            GitService.ensureValidBranchLikeName "Branch name" value
            |> expectOk value)
...
Vitest.describe("GitService.ensureValidPathspec", fun () ->
    let validCases = [|
    ...
    for testName, value, expected in validCases do
        Vitest.test(testName, fun () ->
            GitService.ensureValidPathspec value
            |> expectOk expected)

    Vitest.test("accepts very long pathspec", fun () ->
...
Vitest.describe("GitService.ensureAllowedRemoteUrl", fun () ->
    let validCases = [|
    ...
    for testName, value in validCases do
        Vitest.test(testName, fun () ->
            GitService.ensureAllowedRemoteUrl value
            |> expectOk value)
...
Vitest.describe("GitAuthAdapter.redactToken and redactArgs", fun () ->
    let tokenCases = [|
    ...
    for testName, input, expected in tokenCases do
        Vitest.test(testName, fun () ->
            Vitest.expect(GitAuthAdapter.redactToken input).toBe(expected))

    Vitest.test("keeps null unchanged", fun () ->
        let result: string = GitAuthAdapter.redactToken null
        Vitest.expect(isNull result).toBe(true))

    Vitest.test("redacts tokens in argument arrays", fun () ->
        let redacted =
...
Vitest.describe("GitTokenProvider.tryExtractHostFromRemoteUrl", fun () ->
    ...
    for testName, value, expected in validCases do
        Vitest.test(testName, fun () ->
            GitTokenProvider.tryExtractHostFromRemoteUrl value
            |> expectOk expected)
...
Vitest.describe("GitAuthAdapter.toConfigEntries and buildAuthArgs", fun () ->
    let configCases = [|
...
Vitest.describe("GitService.validateRemoteName", fun () ->
    ...
    for testName, value, expected in validCases do
        Vitest.test(testName, fun () ->
            GitService.validateRemoteName value
            |> expectOk expected)

    Vitest.test("rejects remote with spaces", fun () ->
...
Vitest.describe("GitProvisioningService validation helpers", fun () ->
    Vitest.describe("target path normalization", fun () ->
        ...
        Vitest.test("rejects target path containing null byte", fun () ->
            GitProvisioningService.validateAndNormalizeTargetPathWithResolver id "repo\u0000name"
            |> expectError)

        Vitest.test("normalizes target path via resolver", fun () ->
    ...
    Vitest.describe("clone target emptiness", fun () ->
        let cases = [|
            "fails when existing directory is non-empty", GitProvisioningService.classifyCloneTargetState true 1, false
            "allows existing empty directory target", GitProvisioningService.classifyCloneTargetState true 0, true
    ...
    Vitest.describe("clone branch option assembly", fun () ->
        Vitest.test("returns empty options when branch is missing", fun () ->
            let options = GitProvisioningService.buildCloneBranchOptions None
            Vitest.expect(options).toEqual([||]))

        Vitest.test("includes --branch option pair when branch is provided", fun () ->
...
Vitest.describe("Git IPC provisioning contract reflection", fun () ->
    ...
    Vitest.test("IGitApi.gitCloneRepository uses GitCloneRepositoryRequest argument", fun () ->
    ...
    Vitest.test("GitCloneRepositoryRequest fields match expected schema", fun () ->
...
Vitest.describe("SimpleGit binding surface parity", fun () ->
    Vitest.test("ISimpleGit contains expected upstream method names", fun () ->
        let simpleGitSource = getSimpleGitSource ()
        let interfaceBlock = extractSingleLineBlock "type ISimpleGit =" "\\[<Erase>\\]\\s*type SimpleGit =" simpleGitSource
    ...
    Vitest.test("SimpleGitOptions exposes required option properties including unsafe", fun () ->
        let simpleGitSource = getSimpleGitSource ()
        let optionsBlock = extractSingleLineBlock "type SimpleGitOptions" "type ISimpleGit =" simpleGitSource
    ...
    Vitest.test("SimpleGitOptions abort uses AbortSignal type", fun () ->
        let optionsBlock = extractSingleLineBlock "type SimpleGitOptions" "type ISimpleGit =" (getSimpleGitSource ())
        expectSourceContains optionsBlock "member val abort: IAbortSignal option")

    Vitest.test("SimpleGitOptions binary and errors plugin options are typed", fun () ->
        let optionsBlock = extractSingleLineBlock "type SimpleGitOptions" "type ISimpleGit =" (getSimpleGitSource ())
    ...
    Vitest.test("SimpleGitOptions completion and spawn options are typed", fun () ->
        let optionsBlock = extractSingleLineBlock "type SimpleGitOptions" "type ISimpleGit =" (getSimpleGitSource ())
    ...
    Vitest.test("SimpleGitProgressEvent uses required non-optional fields", fun () ->
        let progressBlock = extractSingleLineBlock "type SimpleGitProgressEvent =" "type SimpleGitProgressHandler" (getSimpleGitSource ())
    ...
    Vitest.test("DiffResult includes the name-status file variant", fun () ->
        expectSourceContains (getSimpleGitSource ()) "abstract member files: U3<DiffResultTextFile, DiffResultBinaryFile, DiffResultNameStatusFile>[]")

    Vitest.test("PullResult includes required summary and created/deleted fields", fun () ->
        let simpleGitSource = getSimpleGitSource ()
    ...
    Vitest.test("DiffResultNameStatusFile optionality matches upstream", fun () ->
        let simpleGitSource = getSimpleGitSource ()
    ...
    Vitest.test("High-impact result fields are non-optional", fun () ->
        let simpleGitSource = getSimpleGitSource ()
    ...
    Vitest.test("Branch summary and branch delete result shapes are strongly typed", fun () ->
        let simpleGitSource = getSimpleGitSource ()
    ...
    Vitest.test("Fetch, push, grep and log response models are typed without obj option placeholders", fun () ->
        let simpleGitSource = getSimpleGitSource ()
    ...
    Vitest.test("Residual required fields stay non-optional and typed", fun () ->
        let simpleGitSource = getSimpleGitSource ()
    ...
    Vitest.test("clearQueue is marked deprecated in binding", fun () ->
        let simpleGitSource = getSimpleGitSource ()
        expectSourceContains simpleGitSource "[<System.Obsolete(\"Deprecated upstream. Removed in v2; prefer abort-plugin configuration for pending task cancellation.\")>]"
        expectSourceContains simpleGitSource "abstract member clearQueue: unit -> ISimpleGit")

    Vitest.test("SimpleGit factory exposes create overloads", fun () ->
        let createMethodCount = Regex.Matches(getSimpleGitSource (), "static member create\\b").Count
