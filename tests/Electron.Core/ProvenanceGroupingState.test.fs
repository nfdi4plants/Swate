...
open Swate.Components.Composite.ProvenanceGrouping
open Vitest

Vitest.describe("ProvenanceGrouping State", fun () ->
    Vitest.test("Layers.ensure returns the same instance for unchanged state", fun () ->
    ...
    Vitest.test("Layers.ensure still prunes pair-scoped state for removed pairs", fun () ->
        let session = sampleSession ()
