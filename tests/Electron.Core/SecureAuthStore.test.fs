...
let private isLowerHex (value: string) =
    value
    |> Seq.forall (fun c -> "0123456789abcdef".IndexOf(c) >= 0)

Vitest.describe("SecureAuthStore.generateAccountId", fun () ->
    Vitest.test("produces distinct hashes for emails that previously collided", fun () ->
    ...
    Vitest.test("normalizes host casing, trailing slash, and email whitespace", fun () ->
        let canonical =
    ...
    Vitest.test("returns a filesystem-safe hex hash", fun () ->
        let accountId =
            SecureAuthStore.generateAccountId "https://datahub.example.org" "user@example.org"
