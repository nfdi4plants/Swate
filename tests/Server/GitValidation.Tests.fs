...
// NOTE by @Freymaurer: These tests are disabled because they are placed in a .NET only test project, while testing fable transpiled behavior. It does not make sense to do this! These MUST be moved to a separate test project, which is fable transpiled.
