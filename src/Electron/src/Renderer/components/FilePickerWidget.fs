module Renderer.components.FilePickerWidget
...
module private FilePickerState =
    ...
    let moveUp (index: int) (fileNames: string list) =
        if index > 0 && index < fileNames.Length then
            swap index (index - 1) fileNames
        else
            fileNames

    let moveDown (index: int) (fileNames: string list) =
        if index >= 0 && index < fileNames.Length - 1 then
            swap index (index + 1) fileNames
        else
            fileNames

    let removeAt (index: int) (fileNames: string list) =
        fileNames
...
module private FilePickerDataSource =
    ...
    let insertFileNames (table: ArcTable) (start: InsertStart) (fileNames: string list) : InsertResult =
    ...
    let insertDataMapFileNames (dataMap: DataMap) (start: InsertStart) (fileNames: string list) : InsertResult =
