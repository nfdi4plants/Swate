module Renderer.components.DataAnnotatorWidget
...
module DataAnnotatorDataSource =
    ...
    let private noDataRowsError = "Parsed file does not contain any data rows."

    let private normalizeSeparator (separator: string) (file: DataAnnotator.DataFile) =
    ...
    let private targetSortKey (target: DataAnnotator.DataTarget) =
        match target with
    ...
    let sortTargets (targets: seq<DataAnnotator.DataTarget>) =
        targets
        |> Seq.sortBy targetSortKey
        |> Seq.toArray

    let selectorsFromTargets (hasHeader: bool) (targets: seq<DataAnnotator.DataTarget>) =
        targets
        |> sortTargets
        |> Array.map (fun target -> target.ToFragmentSelectorString(hasHeader))

    let hasDataRows (parsedFile: DataAnnotator.ParsedDataFile) =
        parsedFile.BodyRows.Length > 0

    let tryParseDataFile (separator: string) (file: DataAnnotator.DataFile) =
        try
            let separator = normalizeSeparator separator file
            let parsed = DataAnnotator.ParsedDataFile.fromFileBySeparator separator file

            if hasDataRows parsed then
                Ok parsed
            else
    ...
    let tryGetTargetHeader (table: ArcTable) (target: DataAnnotator.TargetColumn) =
        match target with
    ...
    let createDataCells (selectors: string[]) (fileName: string) (fileType: string) =
        selectors
    ...
    let applyToTable (table: ArcTable) (input: AnnotationInput) : Result<int, string> =
        try
            match tryGetTargetHeader table input.TargetColumn with
            | Error err -> Error err
            | Ok header ->
                let values = createDataCells input.Selectors input.FileName input.FileType
                table.AddColumn(header, values, forceReplace = true)
                Ok input.Selectors.Length
        with exn ->
            Error exn.Message

    let applyToDataMap (dataMap: DataMap) (input: AnnotationInput) : Result<int, string> =
        try
...
let Main
    ...
    let separatorInput, setSeparatorInput = React.useState DataAnnotatorDataSource.DefaultSeparator
    ...
    let resetAll () =
        setDataFile None
        setParsedFile None
        setSeparatorInput DataAnnotatorDataSource.DefaultSeparator
        clearSelection ()
        ...
        | Some file ->
            ...
            let separatorToUse =
                if System.String.IsNullOrWhiteSpace nextSeparatorInput then
                    if System.String.IsNullOrWhiteSpace file.ExpectedSeparator then
                        DataAnnotatorDataSource.DefaultSeparator
                    else
                        file.ExpectedSeparator
                else
                    nextSeparatorInput
    ...
    let trySubmitAnnotation () =
        ...
        | Some file, Some parsed ->
            ...
            if selectors.Length = 0 then
                ...
                let annotationInput: DataAnnotatorDataSource.AnnotationInput = {
                    Selectors = selectors
                    ...
                    Html.div [
                        ...
                        prop.children [
                            ...
                            | Some parsed ->
                                ...
                                PreviewTable(parsed, selectedTargets, setSelectedTargets)
                        ]
                    ],
                footer =
                    Html.div [
