...
[<ReactComponent>]
let Main (appRootPath: ArcRootPath, workspaceMode: Renderer.Types.WorkspaceMode) =
    match appRootPath with
    | None -> InitTarget()
    | Some _ ->
        match workspaceMode with
        | Renderer.Types.WorkspaceMode.FileExplorer -> ArcTarget()
        | Renderer.Types.WorkspaceMode.ArcObjectExplorer -> ArcObjectExplorerTarget.Main()
