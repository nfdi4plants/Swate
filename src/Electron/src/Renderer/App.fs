...
/// Active view state for file preview - mirrors Swate's Spreadsheet.ActiveView
...
/// ArcFile wrapper for renderer-side usage
type ArcFileState = {
    ArcFile: ArcFiles
...
let Main () =
    ...
    React.useEffect (
        (fun () ->
            if previewData.IsSome then
                console.log "[Renderer] Preview data updated"
        ),
        [| box previewData |]
    )
        ...
        let rec loop (node: FileItemDTO) index =
            ...
            let child =
                ...
                | false, _ ->
                    ...
                    let newNode =
                        FileItemDTO.create (part, newPath, entry.isDirectory, System.Collections.Generic.Dictionary())
    ...
    let getFileTree (fileEntries: FileEntry[]) =
        ...
        let adaptedFileEntires =
            fileEntries |> Array.filter (fun fileEntry -> fileEntry.path <> rootPath)

        let rootElement =
            let tmp = fileEntries |> Array.find (fun fileEntry -> fileEntry.path = rootPath)
            FileItemDTO.create (tmp.name, tmp.path, tmp.isDirectory, System.Collections.Generic.Dictionary())
    ...
    let openARC = if appState.IsInit then openCurrentWindow else openNewWindow
    ...
    let onOpenSelector () =
        promise {
            ...
            | AppState.ARC path ->
                newARCs
                |> Array.map (fun arc -> ARCPointer.create (arc.name, arc.path, arc.path = path))
                |> setRecentARCs
        }
        |> Promise.start
    ...
    let selector =
        Selector.Main(recentARCElements, actionbar, onOpenSelector = onOpenSelector)

    React.useEffectOnce (fun _ -> Remoting.init |> Remoting.buildHandler ipcHandler)

    // Compute the child content outside of the prop.children to avoid Seq operations
