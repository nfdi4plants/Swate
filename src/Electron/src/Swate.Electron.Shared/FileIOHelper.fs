...
module FileContentDTO =
    ...
    let fromArcByPath (path: string) (arc: ARC) =
        ...
        let mutable discFileType: FileContentType option = None

        let arcFileOpt =
            match split with
            | InvestigationPath _ ->
                discFileType <- Some FileContentType.ISA_Investigation
                ArcFiles.Investigation arc |> Some
            | AssayPath p ->
                discFileType <- Some FileContentType.ISA_Assay
            ...
            | StudyPath p ->
                discFileType <- Some FileContentType.ISA_Study
            ...
            | WorkflowPath p ->
                discFileType <- Some FileContentType.ISA_Workflow
