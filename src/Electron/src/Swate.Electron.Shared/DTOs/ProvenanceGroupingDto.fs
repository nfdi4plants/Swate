...
module ProvenanceKindDto =
    ...
    let ofModel (kind: ProvenanceKind) =
        {
            Id = kind.Id
            Label = kind.Label
        }

    let toModel (kind: ProvenanceKindDto) : ProvenanceKind =
        {
            Id = kind.Id
            Label = kind.Label
        }
...
module ProvenanceValueDto =
    ...
    let ofModel value =
        ...
        | ProvenanceValue.Text text ->
            { empty "Text" with Text = Some text }
        | ProvenanceValue.Integer integer ->
            { empty "Integer" with Integer = Some integer }
        | ProvenanceValue.Float float ->
            { empty "Float" with Float = Some float }
        | ProvenanceValue.Term term ->
            { empty "Term" with Term = Some(ProvenanceTermDto.ofModel term) }
