...
/// Discriminator for ArcFile types
...
type PreviewData =
    /// Full ArcFile data as JSON with type discriminator - can be deserialized to proper ARCtrl objects
...
module FileItemDTOExtensions =
    ...
    type FileItemDTO with
        static member create(name: string, path: string, isDirectory: bool, children: Dictionary<string, FileItemDTO>) = {
            name = name
            path = path
            isDirectory = isDirectory
            children = children
