...
let api: IArcVaultsApi = {
        ...
        fun event -> promise {
            try
                ...
                if vault.IsSome && vault.Value.path.IsSome then
                    let recentARCs =
                        ARCHolder.updateRecentARCs vault.Value.path.Value maxNumberRecentARCs
    ...
    getRecentARCs = fun _ -> promise { return recentARCs }
    ...
    openFile =
        fun (path: string) -> promise {
            ...
            | Some vault ->
                ...
                Swate.Components.console.log ($"File type determined: {fileType}")

                // Try to open the correct type first based on filename
                match fileType with
                | "investigation" ->
