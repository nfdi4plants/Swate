...
module ArcVaultExtensions =
    ...
    type ArcVault with
                        ...
                        (fun () ->
                            ...
                            promise {
                                try
                                    try
                                        do!
                                            this.fileTreeWorkQueue.Enqueue(fun () -> promise {
                                                swatelogfn
                                                    this.window.id
        ...
        member this.Startup() = promise {
            ...
            try
                let shouldWaitForWatcher = this.watcher.IsNone
                this.StartFileWatcher()

                if shouldWaitForWatcher then
        ...
        member this.RenameOpenArcRoot(newName: string) : Fable.Core.JS.Promise<Result<string, exn>> = promise {
            ...
            | Some currentPath ->
                ...
                let restoreWatchers arcPath = promise { this.StartFileWatcher() }
