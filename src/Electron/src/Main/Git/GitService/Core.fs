module Main.Git.GitServiceCore
...
open Main.Git.GitServiceFailureClassification
open Main.Git.GitServiceValidation
...
let private gitCommandLineLengthLimit = 24000

let private estimateGitArgumentLength (argument: string) =
    (argument |> Option.ofObj |> Option.defaultValue String.Empty).Length + 3

let private chunkGitPathspecs (command: string[]) (pathSpecs: string[]) =
    let commandLength =
        [| yield! command; yield "--" |] |> Array.sumBy estimateGitArgumentLength

    let chunks = ResizeArray<string[]>()
    let currentChunk = ResizeArray<string>()
    let mutable currentLength = commandLength

    let flushCurrentChunk () =
    ...
    for pathSpec in pathSpecs do
        let nextLength = estimateGitArgumentLength pathSpec

        if currentChunk.Count > 0 && currentLength + nextLength > gitCommandLineLengthLimit then
            flushCurrentChunk ()

        currentChunk.Add pathSpec
        currentLength <- currentLength + nextLength

    flushCurrentChunk ()
    chunks.ToArray()

let private addPathspecChunks (pathSpecs: string[]) (git: ISimpleGit) = promise {
    for chunk in chunkGitPathspecs [| "add" |] pathSpecs do
        let! _ = git.add chunk
        ()
}

let private resetMixedPathspecChunks (pathSpecs: string[]) (git: ISimpleGit) = promise {
    for chunk in chunkGitPathspecs [| "reset"; "mixed" |] pathSpecs do
        let resetOptions = [| yield "--"; yield! chunk |]
        let! _ = git.reset ("mixed", !^resetOptions)
        ()
}

let private rawPathspecChunks (command: string[]) (pathSpecs: string[]) (git: ISimpleGit) = promise {
    let outputs = ResizeArray<string>()

    for chunk in chunkGitPathspecs command pathSpecs do
        let! output = git.raw (withGitPathspecs command chunk)
        outputs.Add output
        ...
        | Ok oversizedPaths ->
            ...
            | None ->
                if pathsToTrack.Length = 0 then
                    ...
                    let! restageResult = runSimpleGit (addPathspecChunks pathsToRestage) git
    ...
    | Ok safePathSpecs ->
        return!
            withLocalGit
                arcPath
                (fun git -> promise {
                    let! diff = rawPathspecChunks [| "diff" |] safePathSpecs git
                    return diff
                })
}
    ...
    | Ok safePathSpecs ->
        return!
            withLocalGit
                arcPath
                (fun git -> promise {
                    let! diff = rawPathspecChunks [| "diff"; "--word-diff=porcelain"; "-U0" |] safePathSpecs git
                    return diff
                })
}
    ...
    | Ok safePathSpecs ->
        return!
            withLocalGit
                arcPath
                (fun git -> promise {
                    do! addPathspecChunks safePathSpecs git
    ...
    | Ok safePathSpecs ->
        return!
            withLocalGit
                arcPath
                (fun git -> promise {
                    do! resetMixedPathspecChunks safePathSpecs git
                    return ()
                })
}
...
let private headPathsForPathspecs (git: ISimpleGit) (pathSpecs: string[]) = promise {
    let! output = rawPathspecChunks [| "ls-tree"; "-r"; "--name-only"; "HEAD" |] pathSpecs git
    return splitGitOutputLines output
}
        ...
        return!
            withLocalGit
                ...
                (fun git -> promise {
                    ...
                    if hasHead then
                        let! _ = rawPathspecChunks [| "reset" |] discardPathSpecs git
                        let! headPaths = headPathsForPathspecs git discardPathSpecs

                        if headPaths.Length > 0 then
                            let! _ = rawPathspecChunks [| "restore"; "--worktree" |] headPaths git
                            ()
                    else
                        let! _ =
                            rawPathspecChunks [| "rm"; "--cached"; "-r"; "--ignore-unmatch" |] discardPathSpecs git

                        ()

                    let! _ = rawPathspecChunks [| "clean"; "-fd" |] discardPathSpecs git
                    return ()
                })
}
