...
module private MainHelpers =
    ...
    let createMinimalTable (model, dispatch) =
...
type Main =
    ...
    [<ReactComponent>]
    static member EmptyTableView(arcFile: ArcFiles, setArcFile: ArcFiles -> unit) =
                ...
                prop.children [
                    CardGrid.CardGrid(
                        React.Fragment [
                            ...
                            // () // TODO: disable if no tables with node columns (Input/output exists in the arcfile)
                            )
                        ]
                    )
                ]
