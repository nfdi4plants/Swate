...
type CancelableErrorModalRequest = {
    ...
    static member create
        ...
        Message = message
        Details = details
        CancelLabel = defaultArg cancelLabel "Cancel"
        DismissLabel = defaultArg dismissLabel "Continue"
        OnCancel = onCancel
        OnDismiss = onDismiss
        Actions = defaultArg actions []
    }
...
type ErrorModalEntry =
    ...
    | Cancelable of CancelableErrorModalRequest
    member this.Id =
        match this with
        | Single request -> request.Id
        | Batch batch -> batch.Id
        | Cancelable request -> request.Id
