...
type InputField =
    ...
    static member TermCombi(v: Term option, setter: Term option -> unit, label: string, rmv, submit, ?autofocus: bool, ?parentOa: OntologyAnnotation) =
        let autofocus = defaultArg autofocus false
        Html.div [
            prop.className "flex flex-col gap-2"
            prop.children [
...
type CompositeCellModal =
    ...
    static member TermModal(oa: OntologyAnnotation, setOa: OntologyAnnotation -> unit, rmv, ?relevantCompositeHeader: CompositeHeader) =
        ...
        let parentOa =
            relevantCompositeHeader
            |> Option.map (fun h -> h.ToTerm())
    ...
    static member UnitizedModal(v: string, oa: OntologyAnnotation, setUnitized: string -> OntologyAnnotation -> unit, rmv, ?relevantCompositeHeader: CompositeHeader) =
        let initTerm = Term.fromOntologyAnnotation oa
    ...
    static member DataModal(v: ARCtrl.Data, setData: ARCtrl.Data -> unit, rmv, ?relevantCompositeHeader: CompositeHeader) =
    ...
    [<ReactComponent>]
    static member CompositeCellModal(compositeCell: CompositeCell, setCell: CompositeCell -> unit, rmv: unit -> unit, ?relevantCompositeHeader: CompositeHeader) =
        ...
        | CompositeCell.Unitized (v, oa) ->
            ...
            CompositeCellModal.UnitizedModal(v, oa, setUnitized, rmv, ?relevantCompositeHeader = relevantCompositeHeader)
