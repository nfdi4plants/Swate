...
module Templates =
    let userTablewithColumns (annoState: Annotation list, fileName: string) =
        ...
        for a in annoState do
            ...
            userTable.AddColumn(header, ResizeArray[a.Search.Body])
...
module DownloadParser =
    ...
    let downloadXlsxProm (fileName, annoState) = promise {
        let! bytes =
            Spreadsheet.Template.toFsWorkbook (Templates.userTemplate (fileName, annoState))
            |> Xlsx.toXlsxBytes
    ...
    let downloadJsonProm (fileName, annoState) = promise {
        let jsonString =
            Template.toJsonString 0 (Templates.userTemplate (fileName, annoState))
