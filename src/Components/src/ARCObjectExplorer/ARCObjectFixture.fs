...
type ARCObjectFixture =
    ...
    static member StoryItemIdCanonicalStudyNote3 = "note:studies:soil-microbiome:field-observations"
    ...
    static member private filterStoryItemsByKinds(visibleKinds: Set<string>, items: FileItem list) =
        let rec loop (item: FileItem) =
            ...
            let itemKind = ARCObjectFixture.tryGetStoryItemKind item
    ...
    static member private storySearchItems(items: FileItem list) =
        ARCObjectFixture.flattenFileItemsWithParent items
        |> List.choose (fun (item, parent) ->
            match ARCObjectFixture.tryGetStoryItemKind item with
            | Some "Group" -> None
            | Some kind ->
                let subtitle =
                    ARCObjectFixture.StoryMeta
    ...
    static member private StoryExample() =
        ...
        let visibleSelectedId =
            Some selectedId
            |> Option.filter (fun itemId -> ARCObjectFixture.tryFindItemAndParent itemId filteredItems |> Option.isSome)
        ...
        let searchItems = ARCObjectFixture.storySearchItems filteredItems
            ...
            prop.children [
                ...
                if isOpen then
                    Html.div [
                        ...
                        prop.children [ ARCObjectFixture.StoryExample() ]
                    ]
            ]
        ]
