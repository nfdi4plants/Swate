...
module private ArcObjectExplorerContentHelper =
    ...
    let arcFileMatchesMetadataNodeKind
        (
    ...
    let optionalTextRow label value =
        value
        |> asOptionalText
        |> Option.map (fun text -> textRow label text)
    ...
    let investigationMetadataRows (investigation: ArcInvestigation) =
        [
            ...
            yield! optionalTextRow "Public Release" investigation.PublicReleaseDate |> Option.toList
            yield!
    ...
    let arcFileMatchesSelectedNodePreviewPath
        (
...
type ArcObjectExplorerContent =
    ...
    [<ReactComponent>]
    static member private DetailedMetadataContent
        (
            arcFile: ArcFiles,
        ...
        | Some selectedNode ->
            let previewArcFile =
                ...
                |> Option.filter (fun state ->
                    ArcObjectExplorerContentHelper.arcFileMatchesSelectedNodePreviewPath(
                        selectedNode,
                        state
                    ))
            ...
            let metadataArcFile =
                ...
                |> Option.filter (fun file ->
                    ArcObjectExplorerContentHelper.arcFileMatchesMetadataNodeKind(
                        selectedNode.kind,
                        file
                    ))
            ...
            let selectedObjectRows =
                ...
                | ArcExplorerNodeKind.Sample, _ ->
                    ...
                    |> Option.map (fun summary ->
                        ArcObjectExplorerContentHelper.sampleSummaryRows(
                            selectedNode.name,
                            summary
                        ))
                | _, Some _ -> None
                | _, None ->
                    ...
                    |> Option.bind (fun file ->
                        ArcObjectExplorerContentHelper.currentPreviewRowsForNode(
                            selectedNode,
                            file
                        ))
            ...
            Html.div [
                ...
                prop.children [
                    ...
                    | Some(PageState.ErrorPage message) -> ArcObjectExplorerContent.ErrorSection("Note Content", message)
                    | _ -> Html.none
