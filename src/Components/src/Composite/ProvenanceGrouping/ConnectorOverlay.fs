...
/// Measures group card positions and builds SVG connector paths between them.
module private ConnectorMeasure =

    let tryNode (container: HTMLElement) nodeId =
        let node: HTMLElement =
            !!container?querySelector($"[data-provenance-group-node='{nodeId}']")

        if isNull node then None else Some node

    let path container connection =
        match
            tryNode container (DragDrop.groupNodeId ProvenanceSide.Input connection.SourceGroupId),
            tryNode container (DragDrop.groupNodeId ProvenanceSide.Output connection.TargetGroupId)
        with
        | Some source, Some target ->
            ...
            let startY =
                sourceRect.top - origin.top + float container.scrollTop + sourceRect.height / 2.

            let endX = targetRect.left - origin.left + float container.scrollLeft

            let endY =
                targetRect.top - origin.top + float container.scrollTop + targetRect.height / 2.
                ...
                for measured in paths do
                    ...
                    Svg.path [
                        ...
                        if defaultArg debug false then
                            svg.custom ("data-testid", "provenance-connection")
