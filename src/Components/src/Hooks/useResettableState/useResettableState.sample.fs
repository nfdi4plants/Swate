        ...
        Html.div [
            ...
            Html.button [
                ...
                prop.onClick (fun _ ->
                    ...
                    if next = items then setItems (Array.append next [| "Zucchini" |]) else setItems next
                )
            ]
        ]
    ]
...
let SampleDatasetSelector() =
    ...
    Html.div [
        ...
        Html.div [ prop.testid "selected-row"; prop.text $"Selected row: {selectedRow}" ]
        Html.div [
            Html.button [
                prop.className "swt:btn swt:btn-sm"
                prop.text "Load Dataset A"
                prop.onClick (fun _ -> setDataset {| id = 1; name = "Dataset A"; gen = 0; rows = [| "Alpha"; "Beta"; "Gamma" |] |})
            ]
            Html.button [
                prop.className "swt:btn swt:btn-sm"
                prop.text "Reload Dataset A"
                prop.onClick (fun _ -> setDataset {| id = 1; name = "Dataset A"; gen = dataset.gen + 1; rows = [| "Alpha"; "Beta"; "Gamma" |] |})
            ]
            Html.button [
                prop.className "swt:btn swt:btn-sm"
                prop.text "Load Dataset B"
                prop.onClick (fun _ -> setDataset {| id = 2; name = "Dataset B"; gen = 0; rows = [| "Delta"; "Epsilon" |] |})
            ]
        ]
    ]
