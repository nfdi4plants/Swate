...
module Extensions =
    ...
    type OntologyAnnotation with
        ...
        member this.ToTerm() =
            let href = this.TermAccessionOntobeeUrl |> Option.whereNot System.String.IsNullOrWhiteSpace
            let description = this.Comments |> Seq.tryFind (fun c -> c.Name = Some OntologyAnnotation.DescriptionCommentKey) |> Option.bind (fun c -> c.Value)
            let isObsolete = this.Comments |> Seq.tryFind (fun c -> c.Name = Some OntologyAnnotation.IsObsoleteCommentKey) |> Option.bind (fun c -> c.Value) |> Option.map System.Boolean.Parse
