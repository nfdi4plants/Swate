...
//         match modal with
//         | ExcelModals.InteropLogging -> Modals.InteropLogging.Main(model.DevState, dispatch)

//     static member TableModal(modal: TableModals, model, dispatch) =
//         match modal with
//         | TableModals.EditColumn columnIndex -> Modals.CompositeHeaderModal.Main(columnIndex, model, dispatch)
//         | TableModals.MoveColumn columnIndex -> Modals.MoveColumn.Main(columnIndex, model, dispatch)
//         | TableModals.ResetTable -> Modals.ResetTable.Main dispatch
//         | TableModals.TermDetails term -> Modals.TermModal.Main(term, dispatch)
...
//         match modal with
//         | GeneralModals.Error exn -> Modals.Error.Main(exn, dispatch)
//         | GeneralModals.Warning msg -> Modals.Warning.Main(msg, dispatch)
//         | GeneralModals.Loading -> Modals.Loading.Modal(dispatch)
...
//                 | { ActiveModal = None } -> Html.none
//                 | { ActiveModal = Some modal } ->
//                     match modal with
//                     | GeneralModal m -> ModalProvider.GeneralModal(m, model, dispatch)
//                     | TableModal m -> ModalProvider.TableModal(m, model, dispatch)
//                     | ExcelModal m -> ModalProvider.ExcelModal(m, model, dispatch)
//                     | Force ele -> ele
//             ]
//         ]