...
type FooterTabs =
    static member Table (mouseX, mouseY, tableIndex: int, startEdit: _ -> unit, dispatch: Messages.Msg -> unit) =
        let delete = fun _ -> Spreadsheet.RemoveTable tableIndex |> Messages.SpreadsheetMsg |> dispatch
    ...
    static member Plus (mouseX, mouseY, dispatch: Messages.Msg -> unit) =
        let addTable = fun _ -> SpreadsheetInterface.CreateAnnotationTable false |> Messages.InterfaceMsg |> dispatch
        let addDataMap = fun _ -> SpreadsheetInterface.UpdateDatamap (DataMap.init() |> Some) |> Messages.InterfaceMsg |> dispatch
        let children (rmv: unit -> unit) : ReactElement seq = [
    ...
    static member DataMap (mouseX, mouseY, dispatch: Messages.Msg -> unit) =
        let delete = fun _ -> SpreadsheetInterface.UpdateDatamap None |> Messages.InterfaceMsg |> dispatch
        let children (rmv: unit -> unit) : ReactElement seq = [
