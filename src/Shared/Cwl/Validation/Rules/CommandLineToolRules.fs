...
module Swate.Components.Shared.Cwl.Validation.Rules.CommandLineToolRules
            ...
            Run =
                fun ctx ->
                    ...
                    | CWLProcessingUnit.CommandLineTool td when includeAdvisoryIssues ctx.Mode ->
                        ...
                        | None -> [
                            issue r Warning "baseCommand" "CommandLineTool has no baseCommand."
                          ]
                        | Some bc when bc.Count = 0 -> [ issue r Warning "baseCommand" "baseCommand is empty." ]
                        | _ -> []
                    | _ -> []
    }
...

    {
        r with
            Run =
                fun ctx ->
                    match ctx.ProcessingUnit with
                    | CWLProcessingUnit.CommandLineTool td when
                        includeAdvisoryIssues ctx.Mode && ctx.CwlVersion = DefaultCwlVersion
                        ->
                        td.Inputs
                            ...
                            | Some ib when ib.Position.IsNone ->
                                Some(
                                    issue
                                        r
                                        Info
                                        (sprintf "inputs[%d].inputBinding" i)
