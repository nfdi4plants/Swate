...
module CWLBuilder.Validation.Rules.RequirementsRules
            ...
            if includeAdvisoryIssues ctx.Mode then
                ...
                |> List.mapi (fun i req ->
                    ...
                    | Requirement.DockerRequirement dr ->
                        ...
                        | None | Some "" ->
                            Some (issue r Warning (sprintf "requirements[%d]" i)
            ...
            if includeAdvisoryIssues ctx.Mode then
                ...
                |> List.choose (fun (name, items) ->
                    if items.Length > 1 then
                        Some (issue r Warning "requirements"
            ...
            if includeAdvisoryIssues ctx.Mode then
                ...
                |> List.mapi (fun i req ->
                    ...
                    | Requirement.WorkReuseExpressionRequirement expression ->
                        Some (issue r Warning (sprintf "requirements[%d]" i)
                                (sprintf "WorkReuse uses expression value '%s'. Verify target runner support." expression))
                    | Requirement.NetworkAccessExpressionRequirement expression ->
                        Some (issue r Warning (sprintf "requirements[%d]" i)
                                (sprintf "NetworkAccess uses expression value '%s'. Verify target runner support." expression))
                    | _ -> None)
                |> List.choose id
            else [] }
        ...
        Run = fun ctx ->
            ...
            |> List.mapi (fun i req ->
                ...
                | Requirement.ToolTimeLimitRequirement (ToolTimeLimitSeconds seconds) when seconds < 0L ->
                    Some (issue r Error (sprintf "requirements[%d]" i)
            ...
            |> List.mapi (fun i req ->
                ...
                | Requirement.ResourceRequirement resource ->
                    ...
                    |> List.choose (fun (minField, maxField) ->
                        match tryGetResourceNumeric resource minField, tryGetResourceNumeric resource maxField with
                        | Some minValue, Some maxValue when maxValue < minValue ->
                            Some (issue r Error (sprintf "requirements[%d].%s" i maxField)
                ...
                | Requirement.EnvVarRequirement envDefs ->
                    ...
                    |> Seq.choose (fun (name, matches) ->
                        ...
                        if count > 1 && System.String.IsNullOrWhiteSpace name |> not then
                            Some (issue r Error (sprintf "requirements[%d].envDef" i)
