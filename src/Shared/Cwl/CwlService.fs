...
module CWLBuilder.CwlHandling.CwlService
...
let private annotateResolvedWorkflowStepsFromRaw (rawWorkflow: CWLWorkflowDescription) (resolvedWorkflow: CWLWorkflowDescription) (workflowPath: string) =
    ...
    let rawRunByStepId =
        ...
        |> Seq.choose (fun step ->
            match step.Run with
            | WorkflowStepRun.RunString runString when System.String.IsNullOrWhiteSpace runString |> not -> Some (step.Id, runString)
            | _ -> None
        )
        |> Map.ofSeq
    ...
    for step in resolvedWorkflow.Steps do
        ...
        | Some runString ->
            setWorkflowStepExternalRunMetadata workflowPath step runString
        | None ->
            ()
...
let private applyWorkflowRunReferenceWriteback
    ...
    for step in workflow.Steps do
        ...
        | Some runString ->
            step.Run <- WorkflowStepRun.RunString runString
        | None ->
            ()
...
let tryLoadToEditor (yaml: string) (filePath: string) : Result<EditorState, string> =
    tryLoad yaml
    |> Result.map (fun pu -> fromLoaded pu filePath)
...
let save (pu: CWLProcessingUnit) : string =
    Encode.encodeProcessingUnit pu
...
let saveFromEditor (state: EditorState) : string =
    save state.ProcessingUnit
...
let saveFromEditorForPath (state: EditorState) (targetPath: string) : string =
    ...
    | CWLProcessingUnit.Workflow workflow ->
        let originalRuns =
            workflow.Steps
            |> Seq.map (fun step -> step, step.Run)
            |> Seq.toArray
...
let roundtrip (yaml: string) : Result<string, string> =
    tryLoad yaml
    |> Result.map save
