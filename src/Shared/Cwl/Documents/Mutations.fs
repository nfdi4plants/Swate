module CWLBuilder.Electron.Renderer.Documents.Mutations

open CWLBuilder.Electron.Renderer.Documents.Common
open CWLBuilder.Electron.Renderer.Documents.Types

let private append item items =
    items @ [ item ]

let addInput (input: InputModel) (inputs: InputModel list) =
    append input inputs

let updateInput (inputId: InputId) update (inputs: InputModel list) =
    inputs
    |> List.map (fun (input: InputModel) ->
        if input.Id = inputId then update input else input
    )

let removeInput (inputId: InputId) (inputs: InputModel list) =
    inputs
    |> List.filter (fun (input: InputModel) -> input.Id <> inputId)

let addOutput (output: OutputModel) (outputs: OutputModel list) =
    append output outputs

let updateOutput (outputId: OutputId) update (outputs: OutputModel list) =
    outputs
    |> List.map (fun (output: OutputModel) ->
        if output.Id = outputId then update output else output
    )

let removeOutput (outputId: OutputId) (outputs: OutputModel list) =
    outputs
    |> List.filter (fun (output: OutputModel) -> output.Id <> outputId)

let addRequirementNode (node: RequirementNode) (nodes: RequirementNode list) =
    append node nodes

let updateRequirementNode (requirementNodeId: RequirementNodeId) update (nodes: RequirementNode list) =
    nodes
    |> List.map (fun (node: RequirementNode) ->
        if node.Id = requirementNodeId then update node else node
    )
...
let addWorkflowStep (step: WorkflowStepModel) (model: WorkflowModel) =
    { model with Steps = append step model.Steps }

let updateWorkflowStep (stepId: StepId) update (model: WorkflowModel) =
    { model with
        Steps =
            model.Steps
            |> List.map (fun (step: WorkflowStepModel) ->
                if step.Id = stepId then update step else step
            ) }
...
let removeWorkflowStep (stepId: StepId) (model: WorkflowModel) =
    { model with
        Steps =
            model.Steps
            |> List.filter (fun (step: WorkflowStepModel) -> step.Id <> stepId) }

let addStepInput (stepId: StepId) (stepInput: StepInputModel) (model: WorkflowModel) =
    updateWorkflowStep stepId (fun step -> { step with Inputs = append stepInput step.Inputs }) model

let updateStepInput (stepId: StepId) (stepInputId: StepInputId) update (model: WorkflowModel) =
    updateWorkflowStep stepId (fun step ->
        { step with
            Inputs =
                step.Inputs
                |> List.map (fun (input: StepInputModel) ->
                    if input.Id = stepInputId then update input else input
                ) }
    ) model
...
let addStepOutput (stepId: StepId) (stepOutput: StepOutputModel) (model: WorkflowModel) =
    updateWorkflowStep stepId (fun step -> { step with Outputs = append stepOutput step.Outputs }) model
...
let setCommandLineToolInputs (inputs: InputModel list) (model: CommandLineToolModel) : CommandLineToolModel =
    { model with Inputs = inputs }

let setCommandLineToolOutputs (outputs: OutputModel list) (model: CommandLineToolModel) : CommandLineToolModel =
    { model with Outputs = outputs }

let setWorkflowInputs (inputs: InputModel list) (model: WorkflowModel) : WorkflowModel =
    { model with Inputs = inputs }
...
let updateDocumentInputs (update: InputModel list -> InputModel list) (document: EditorDocument) =
    match document with
    | CommandLineToolDoc model -> CommandLineToolDoc { model with Inputs = update model.Inputs }
    | WorkflowDoc model -> WorkflowDoc { model with Inputs = update model.Inputs }
    | ExpressionToolDoc model -> ExpressionToolDoc { model with Inputs = update model.Inputs }
    | OperationDoc model -> OperationDoc { model with Inputs = update model.Inputs }

let updateDocumentOutputs (update: OutputModel list -> OutputModel list) (document: EditorDocument) =
    match document with
    | CommandLineToolDoc model -> CommandLineToolDoc { model with Outputs = update model.Outputs }
    | WorkflowDoc model -> WorkflowDoc { model with Outputs = update model.Outputs }
    | ExpressionToolDoc model -> ExpressionToolDoc { model with Outputs = update model.Outputs }
    | OperationDoc model -> OperationDoc { model with Outputs = update model.Outputs }
