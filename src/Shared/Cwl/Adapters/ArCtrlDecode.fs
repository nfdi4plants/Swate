...
let private addField key value fields =
    if String.IsNullOrWhiteSpace value then
        ...
        Map.add key value fields

let private addOptionalField key value fields =
    value
    |> Option.map (fun text -> addField key text fields)
    |> Option.defaultValue fields

let private joinLines values =
    values |> Seq.toList |> String.concat "\n"

let private joinOutputSource (value: OutputSource) = value.AsValues() |> Seq.toList
...
let private decodeOutputBinding (binding: OutputBinding option) =
    binding |> Option.map (fun item -> { Glob = item.Glob })
...
let private decodeRequirement (requirement: Requirement) =
    let fields =
        ...
        | Requirement.LoadListingRequirement value ->
            Map.ofList [
                "loadListing", LoadListingEnum.toCwlString value.LoadListing
            ]
        ...
        | Requirement.ShellCommandRequirement -> Map.empty
        ...
        | Requirement.InplaceUpdateRequirement value ->
            Map.ofList [
                "inplaceUpdate", if value.InplaceUpdate then "true" else "false"
            ]
        ...
        | Requirement.StepInputExpressionRequirement -> Map.empty
...
let private decodeHint (hint: HintEntry) =
    match hint with
    | KnownHint requirement -> decodeRequirement requirement
    | UnknownHint value -> {
        Id = newRequirementNodeId ()
        Key = "unknown-hint"
        Fields = Map.empty |> addOptionalField "class" value.Class
      }
...
and private decodeWorkflowRun (step: WorkflowStep) =
    ...
    | None ->
        ...
        | WorkflowStepRun.RunCommandLineTool toolObj ->
            InlineCommandLineTool(decodeCommandLineTool (unbox<CWLToolDescription> toolObj))
        | WorkflowStepRun.RunWorkflow workflowObj ->
            InlineWorkflow(decodeWorkflow (unbox<CWLWorkflowDescription> workflowObj))
        | WorkflowStepRun.RunExpressionTool expressionToolObj ->
            InlineExpressionTool(decodeExpressionTool (unbox<CWLExpressionToolDescription> expressionToolObj))
        | WorkflowStepRun.RunOperation operationObj ->
            InlineOperation(decodeOperation (unbox<CWLOperationDescription> operationObj))
