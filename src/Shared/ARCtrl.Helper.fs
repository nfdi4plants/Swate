...
module Extensions =
    ...
    type DataMap with
        ...
        member this.GetCell(columnIndex: int, rowIndex: int) =
            ...
            | DataMapIndices.FilePath -> doptstr r.FilePath |> CompositeCell.FreeText
            | DataMapIndices.Selector -> doptstr r.Selector |> CompositeCell.FreeText
            | DataMapIndices.SelectorFormat -> doptstr r.SelectorFormat |> CompositeCell.FreeText
            | DataMapIndices.Format -> doptstr r.Format |> CompositeCell.FreeText
        ...
        member this.SetCell(columnIndex: int, rowIndex: int, cell: CompositeCell) =
            let r = this.DataContexts.[rowIndex]
            match columnIndex with
            | DataMapIndices.FilePath -> r.FilePath <- Some cell.AsFreeText
            | DataMapIndices.Selector -> r.Selector <- Some cell.AsFreeText
            | DataMapIndices.SelectorFormat -> r.SelectorFormat <- Some cell.AsFreeText
            | DataMapIndices.Format -> r.Format <- Some cell.AsFreeText
        ...
        static member getHeader(columnIndex: int) =
            match columnIndex with
            | DataMapIndices.FilePath -> CompositeHeader.FreeText "File Path"
            | DataMapIndices.Selector -> CompositeHeader.FreeText "Selector"
            | DataMapIndices.SelectorFormat -> CompositeHeader.FreeText "Selector Format"
            | DataMapIndices.Format -> CompositeHeader.FreeText "Format"
            ...
            | DataMapIndices.Explication -> CompositeHeader.Parameter (OntologyAnnotation "Explication")
            | DataMapIndices.Unit -> CompositeHeader.Parameter (OntologyAnnotation "Unit")
            | DataMapIndices.ObjectType -> CompositeHeader.Parameter (OntologyAnnotation "Object Type")
            | i -> failwithf "Invalid column index for DataMap: %i" i
